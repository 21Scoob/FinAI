...
export default function Signup() {
  ...
  const handleSubmit = async (event) => {
    ...
    // Blocul tău try/catch este perfect!
  ...
  return (
    // ... (TOT JSX-ul tău rămâne ABSOLUT IDENTIC) ...
    <div className="flex flex-col min-h-screen bg-zinc-50 dark:bg-black text-white ">
      <Topbar />
      <main className="flex flex-grow flex-col justify-center items-center p-4 mb-8">
        <div className="w-full max-w-sm bg-zinc-900 rounded-lg p-8 shadow-xl">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              <button
                type="submit"
                className="w-full py-2 px-4 bg-white text-black ... cursor-pointer disabled:cursor-not-allowed disabled:opacity-75"
                disabled={isLoading}
              >
                {isLoading ? "Se creează..." : "Continue"}
              </button>
            </div>
          </form>
          {/* ... (restul JSX-ului tău) ... */}
...
// ... (componentele tale LogoIcon, GoogleIcon etc. rămân neschimbate) ...
